...
for diseaseno = 1:length(diseases)
    for c = 1:5
        ...
        figure(c), 
            ...
            subplot(2,4,5), imshow(bin_red), title(strcat('Red-8m(0-75) connected regions = ', int2str(red_8m))),
            subplot(2,4,6), imshow(bin_green), title(strcat('Green-8m(150-250) connected regions = ', int2str(green_8m))),
            subplot(2,4,7), imshow(img_seg&bin_red), title(strcat('Intersection 8m = ', int2str(edge_8m))),
            subplot(2,4,8), imshow(bin_red&bin_green), title(strcat('Intersection 8m = ', int2str(inter_8m)));
